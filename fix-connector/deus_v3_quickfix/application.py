...
from deus_v3_quickfix.helpers import log, setup_logger, read_FIX_message, extract_message_field_value, datetime_to_str

from deus_v3_quickfix.order import order
from deus_v3_quickfix.sender import sender
from deus_v3_quickfix.history import history
from deus_v3_quickfix.execution_report import execution_report
...
class application(fix.Application):
    ...
    def onCreate(self, sessionID):
        ...
        if self.settings.get(sessionID).getString('SessionQualifier') == 'Quote':
            ...
            ses = self.settings.get(sessionID)
            acct = ses.getString('Account')
            self.sender.set_account(acct)
    ...
    def toAdmin(self, message, sessionID):
        ...
        if msgType.getValue() == fix.MsgType_Logon:
            ...
            try:
                account = self.settings.get(sessionID).getString('Account')
            except: pass
    ...
    def fromAdmin(self, message, sessionID):
        ...
        if msgType.getValue() == fix.MsgType_Heartbeat:
            ...
            if self.verbose:
                print(base_str+ 'Heartbeat')
            
        elif msgType.getValue() == fix.MsgType_Logon:
            
            print(base_str + 'Received LOGON Request')
            
        elif msgType.getValue() == fix.MsgType_Logout:
            
            print(base_str + 'Logout.')
